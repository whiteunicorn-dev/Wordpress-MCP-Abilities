...
#       F.4  fs-delete-file     - Delete a file
# ---------------------------------------------------------
...
add_action( 'wp_abilities_api_init', function() {
    ...
    wp_register_ability( 'wua-mcp-filesystem/fs-delete-file', [
        ...
        'execute_callback' => function( $input ) {
            ...
            if ( strpos( $path, '/' ) !== 0 ) $path = trailingslashit( WP_CONTENT_DIR ) . ltrim( $path, '/' );
            $real = realpath( $path );
            if ( $real === false ) return new WP_Error( 'file_not_found', 'File not found: ' . $path );
            if ( strpos( $real, WP_CONTENT_DIR ) === false ) return new WP_Error( 'path_not_allowed', 'Deletes are only allowed within the wp-content directory.' );
            if ( is_dir( $real ) ) return new WP_Error( 'is_directory', 'Path is a directory, not a file: ' . $real );
            if ( ! is_writable( $real ) ) return new WP_Error( 'not_writable', 'File is not writable/deletable: ' . $real );
            $deleted = unlink( $real );
            if ( ! $deleted ) return new WP_Error( 'delete_failed', 'Failed to delete file: ' . $real );
